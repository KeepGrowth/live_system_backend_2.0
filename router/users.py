...
async def update_info_api(
    ...
    :return:
    """
    updated_user = await users.update_user(db, user_id=current_user_id,
                                           user_update=info.model_dump(exclude_none=True, exclude_unset=True))

    updated_user = SafeUserResponse().model_validate(updated_user)
    return Result.success(msg="修改个人信息成功", data=updated_user)
...
async def get_user_detail(
    ...
    return Result.success(msg="获取用户信息成功", data=data)
